...
function main(){
 ...
 fs.writeFileSync(beerOutput,stringifyCsv([['Beer Name','Draft 10 oz Price','Draft 10 oz Happy Hour','Draft 16 oz Price','Draft 16 oz Happy Hour','Can 12 oz Price','Can 12 oz Happy Hour','Can 24 oz Price','Can 24 oz Happy Hour','Obsolete 20 oz Price (Review Only)'],...beer.map(r=>[r.item_name,r.draft_10oz_price,r.draft_10oz_happy_hour,r.draft_16oz_price,r.draft_16oz_happy_hour,r.can_12oz_price,r.can_12oz_happy_hour,r.can_24oz_price,r.can_24oz_happy_hour,r.obsolete_20oz_price])]));
 fs.writeFileSync(liquorOutput,stringifyCsv([['Item Name','Base Price ($)','Happy Hour ]));
...
,'Liquor Type'],...liquor.map(r=>[r.item_name,r.base_price,r.happy_hour_price,r.liquor_type])]));
 console.log(JSON.stringify({input,beerOutput,liquorOutput,beerProducts:beer.length,liquorItems:liquor.length,note:'10 oz draft preserved explicitly; not mapped to Toast stock 8 oz.'},null,2));
}
try{main();}catch(error){console.error('Error: '+error.message);process.exitCode=1;}
